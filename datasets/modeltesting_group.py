...
validationset=pandas.read_csv('./automated_data/processed_data_testing_againstgrouponly.csv')
...
for x in range(len(validationset)):
    data['data'].append({'jobrequirements':validationset.at[x,'jobrequirements']})
...
for x in range(len(result['result'])):
    if int(result['result'][x])!=int(validationset.at[x,'againstgroup']):
        nwrong+=1
